...
ALTER TABLE "public"."cart_temp" OWNER TO "postgres";
...
CREATE SEQUENCE IF NOT EXISTS "public"."reservaciones_id_reservaciones_seq"
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER TABLE "public"."reservaciones_id_reservaciones_seq" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."reservaciones" ALTER COLUMN "id_reservaciones" SET DEFAULT "nextval"('"public"."reservaciones_id_reservaciones_seq"'::"regclass");
...
ALTER TABLE ONLY "public"."cart_temp"
    ADD CONSTRAINT "cart_temp_pkey" PRIMARY KEY ("id_cart_temp");
...
ALTER TABLE ONLY "public"."reservaciones"
    ADD CONSTRAINT "reservaciones_pkey" PRIMARY KEY ("id_reservaciones");
...
CREATE OR REPLACE TRIGGER "set_updated_at" BEFORE UPDATE ON "public"."user_addresses" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
ALTER TABLE ONLY "public"."cart_temp"
    ADD CONSTRAINT "cart_temp_product_id_fkey" FOREIGN KEY ("product_id") REFERENCES "public"."products"("id") ON DELETE CASCADE;
...
CREATE POLICY "Allow insert payments from backend" ON "public"."payments" FOR INSERT WITH CHECK (true);



CREATE POLICY "Allow insert payments from webhook" ON "public"."payments" FOR INSERT WITH CHECK (true);
...
CREATE POLICY "CartTemp: solo por session_id" ON "public"."cart_temp" USING (("session_id" = "current_setting"('request.jwt.claim.session_id'::"text", true)));
...
CREATE POLICY "Insert own addresses" ON "public"."user_addresses" FOR INSERT WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "OrderItems: Users can view own items" ON "public"."order_items" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."orders" "o"
  WHERE (("o"."id" = "order_items"."order_id") AND ("auth"."uid"() = "o"."user_id")))));



CREATE POLICY "OrderItems: service_role puede insertar" ON "public"."order_items" FOR INSERT TO "service_role" WITH CHECK (true);
...
CREATE POLICY "Orders: Admins can update" ON "public"."orders" FOR UPDATE USING (("auth"."role"() = 'admin'::"text"));



CREATE POLICY "Orders: Admins can view all" ON "public"."orders" FOR SELECT USING (("auth"."role"() = 'admin'::"text"));



CREATE POLICY "Orders: Users can view own" ON "public"."orders" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Orders: service_role puede insertar" ON "public"."orders" FOR INSERT TO "service_role" WITH CHECK (true);



CREATE POLICY "Payments: service_role puede insertar" ON "public"."payments" FOR INSERT TO "service_role" WITH CHECK (true);



CREATE POLICY "Select own addresses" ON "public"."user_addresses" FOR SELECT USING (("user_id" = "auth"."uid"()));



CREATE POLICY "Update own addresses" ON "public"."user_addresses" FOR UPDATE USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "insert_own_addresses" ON "public"."user_addresses" FOR INSERT WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "select_own_addresses" ON "public"."user_addresses" FOR SELECT USING (("user_id" = "auth"."uid"()));



CREATE POLICY "update_own_addresses" ON "public"."user_addresses" FOR UPDATE USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
